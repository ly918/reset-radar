...
@MainActor final class DemoModel: ObservableObject {
    ...
    @Published var feedback = "离线就绪 · 未产生网络请求"
    @Published var preview = "尚无通知预览"
    ...
    static func makeEvents(_ now: Date) -> [ResetEvent] {
        ...
        for i in 0...30 {
            result.append(ResetEvent(id: "demo-reset-\(i)", occurredAt: time, observedAt: time,
                                     verifiedAt: time, source: "本地合成记录 demo-reset-\(i)"))
        ...
        result.append(ResetEvent(id: "demo-credit", type: .bankedCredit,
                                 ...
                                 verifiedAt: now.addingTimeInterval(-3 * 3600), scope: "演示套餐 · 可稍后领取",
                                 source: "本地合成记录 demo-credit"))
        return result
    }
    ...
    var signalLabel: String {
        if scenario == .unconfigured { return "未分析" }
        if now.timeIntervalSince(start.addingTimeInterval(-3600)) >= 6 * 3600 { return "计划窗口已过 · 示例" }
        return "Strong · 示例"
    }
    var values: [String] {
    ...
    var status: String {
        ...
        default: return forecast.reason
        }
    }
    var drought: String {
        guard let hours = history.age else { return "未知" }
    ...
    func confirm() {
        if !ledger.events.contains(where: { $0.id == "demo-candidate" }) {
            ledger = EventLedger(events: ledger.events + [ResetEvent(id: "demo-candidate", status: .candidate,
                                occurredAt: now.addingTimeInterval(-1800), observedAt: now, source: "合成公告：Codex limits have now been reset.")])
        }
    ...
    func previewWatch() {
        ...
        preview = "\(decision.reason)：未来 24 小时估计概率升至 78%。\n固定阈值场景，仅应用内预览。"
    }
}
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if args.contains("--diagnose-analysis") {
            Task {
                ...
                print(model.analysisStatus)
                exit(model.analysisStatus.contains("分析完成") ? 0 : 1)
            }
            return
        }
...
struct DemoBadge: View {
    var body: some View {
        HStack(spacing: 6) {
            ...
            Text("Demo · 演示数据").fontWeight(.semibold)
            Spacer()
        ...
        VStack(spacing: 0) {
            VStack(spacing: 12) {
                HStack {
                    ...
                    Spacer()
                    Button { activate("settings") } label: { Image(systemName: "gearshape") }.help("设置")
                ...
                VStack(alignment: .leading, spacing: 16) {
                    VStack(spacing: 5) {
                        Text(model.values[1]).font(.system(size: 48, weight: .medium, design: .rounded)).monospacedDigit()
                            .accessibilityLabel("未来24小时估计概率 \(model.values[1])，演示数据")
                        Text("未来 24 小时").font(.subheadline)
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        Text("可能时段").font(.subheadline.weight(.semibold))
                        if model.scenario == .overview {
                            Text("明日 00:00–06:00 · 固定演示时段").font(.caption).foregroundStyle(.secondary)
                        } else if let start = model.forecast.likelyStartHour, model.scenario != .offline {
                            Text("\(model.now.addingTimeInterval(Double(start) * 3600).formatted(date: .abbreviated, time: .shortened)) 起 6 小时 · 本地时间")
                                .font(.caption).foregroundStyle(.secondary)
                        } else { Text("尚无清晰时间窗口").font(.caption).foregroundStyle(.secondary) }
                    }
                    DisclosureGroup("为什么这样显示", isExpanded: $why) {
                        Text(model.scenario == .overview ? "本页 46% / 72% / 88% 是界面演示值。请在底部切换到“合成历史 · 实际计算”查看本地算法输出。" : "合成已完成间隔 \(model.history.intervals.count) 个；尾部外推 \(model.forecast.tailHours.count) 小时。未知状态不显示为 0%。参数尚未用真实历史校准。")
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text(model.scenario == .unconfigured ? "连接未配置，暂无实时信号。以下仅为合成文本。" : "虚构计划：在未来数小时调整 Codex 用量").font(.caption)
                        Button { evidence = "虚构作者 · demo-post-001\n\n\(posts[0])\n\n此为本地合成文本，不是 Tibo 的真实发言，没有原帖链接。" } label: {
                            Text("“\(posts[0])”").font(.caption).lineLimit(2).multilineTextAlignment(.leading)
                        }.buttonStyle(.plain).foregroundStyle(.secondary)
                        Text("合成帖子 · 点击查看证据").font(.caption2).foregroundStyle(.tertiary)
                    }
                    if model.scenario == .candidate {
                        VStack(alignment: .leading, spacing: 8) {
                            Text("疑似已重置 · 需要核验").font(.subheadline.weight(.semibold))
                            Text("合成公告：Codex limits have now been reset.\n类型：global reset；时间：测试时钟前30分钟；范围：演示全局。")
                                .font(.caption).foregroundStyle(.secondary)
                            HStack {
                                Button("核验已发生") { model.confirm() }.disabled(model.ledger.events.contains { $0.id == "demo-candidate" && $0.status != .candidate })
                                Button("撤回确认") { model.retract() }.disabled(!model.ledger.events.contains { $0.id == "demo-candidate" && $0.status == .verified })
                            }
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        ...
                        if let percentile = model.history.percentile {
                            Text("长于 \(Int(percentile))% 的已记录历史间隔 · 合成样本").font(.caption).foregroundStyle(.secondary)
                        } else { Text("距最近已知重置 · 样本不足时不显示百分位").font(.caption).foregroundStyle(.secondary) }
                    }
                    Divider()
                    VStack(alignment: .leading, spacing: 10) {
                        Text("Recent from Tibo · 虚构帖子").font(.subheadline.weight(.semibold))
                        ForEach(Array(posts.prefix(allPosts ? 10 : 3).enumerated()), id: \.offset) { i, post in
                            Button { evidence = "demo-post-\(i + 1) · 合成文本\n\n\(post)\n\n没有真实来源 URL，不发出网络请求。" } label: {
                        ...
                        Button(allPosts ? "收起" : "展开更多") { allPosts.toggle() }.font(.caption)
                    }
                    Divider()
                    VStack(alignment: .leading, spacing: 10) {
                        HStack { Text("Recent Resets").font(.subheadline.weight(.semibold)); Spacer(); Text("合成已核验记录").font(.caption2).foregroundStyle(.secondary) }
                        if model.events.isEmpty { Text("暂无已核验重置记录").font(.caption).foregroundStyle(.secondary) }
                        ForEach(Array(model.events.prefix(allResets ? 50 : 5))) { event in
                            Button { evidence = "\(event.id)\n\n类型：\(event.type.rawValue)\n适用范围：\(event.scope)\n来源：\(event.source)\n\n仅用于本地演示，未写入真实数据集。" } label: {
                                HStack(alignment: .top) {
                                    VStack(alignment: .leading, spacing: 3) {
                                        Text(event.type == .bankedCredit ? "Banked credit · 可稍后领取" : "Global reset")
                                        Text(event.occurredAt?.formatted(date: .abbreviated, time: .shortened) ?? "时间未知").foregroundStyle(.secondary)
                                    }; Spacer(); Image(systemName: "doc.text.magnifyingglass").foregroundStyle(.secondary)
                                }
                            }.buttonStyle(.plain).font(.caption)
                        }
                        if model.events.count > 5 { Button(allResets ? "收起记录" : "查看全部 \(model.events.count) 条") { allResets.toggle() }.font(.caption) }
                    }
                    Divider()
                    VStack(alignment: .leading, spacing: 8) {
                        Text("本地预演控制").font(.caption.weight(.semibold)).foregroundStyle(.secondary)
                        Picker("场景", selection: $model.scenario) { ForEach(DemoScenario.allCases) { Text($0.rawValue).tag($0) } }.labelsHidden()
                        HStack {
                            Button("推进 6 小时") { model.advance() }
                            Button("通知预览") { model.previewWatch(); showPreview = true }
                        }.font(.caption)
                        Text(model.now.formatted(date: .abbreviated, time: .shortened) + " · 测试时钟").font(.caption2).foregroundStyle(.secondary)
                    }
            ...
            HStack {
                ...
                Spacer(minLength: 4)
                Button("刷新") { model.refresh() }.font(.caption)
                Button("退出") { NSApplication.shared.terminate(nil) }.font(.caption)
            ...
            .sheet(item: Binding(get: { evidence.map(Evidence.init) }, set: { evidence = $0?.text })) { item in
                VStack(alignment: .leading, spacing: 16) {
                    DemoBadge(); Text("来源与证据").font(.headline)
                    Text(item.text).font(.body).textSelection(.enabled)
                    Button("完成") { evidence = nil }.keyboardShortcut(.defaultAction)
                }.padding(24).frame(width: 340)
            }
            .sheet(isPresented: $showPreview) {
                VStack(alignment: .leading, spacing: 16) {
                    DemoBadge(); Label("Reset Watch", systemImage: "cloud.sun").font(.headline)
                    Text(model.preview); Text("系统通知提交次数：0").font(.caption).foregroundStyle(.secondary)
                    Button("完成") { showPreview = false }.keyboardShortcut(.defaultAction)
                }.padding(24).frame(width: 340)
            }
    }
...
struct OnboardingView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            Text(step == 1 ? "少刷 X，看看 Reset 天气。" : step == 2 ? "连接 AI" : "连接动态与完成").font(.title2.weight(.semibold))
            Text(step == 1 ? "菜单栏常驻 · 公开信息估计 · 本机保存\n当前是离线技术预演，所有帖子和概率都为演示。" : step == 2 ? "AI 未配置。此轮可跳过并体验离线演示。\n真实连接请到设置的“真实连接”页配置；此向导只演示离线流程。" : "动态未配置 · AI 已跳过\n当前不会实时观察 Tibo，也不会提交系统通知。")
                .foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true)
            Text("预测在本机计算；后续启用 AI 时，公开帖文会发送至 OpenAI，API 可能单独收费。")
                .font(.caption).foregroundStyle(.secondary)
            Spacer()
            HStack {
                if step > 1 { Button("上一步") { step -= 1 } }
                Button("先体验") { openWindow(id: "rehearsal"); dismiss() }
                Spacer()
                Button(step == 3 ? "开始使用" : step == 2 ? "暂时跳过" : "开始设置") {
...
struct DemoSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            DemoBadge()
            Text("设置与预演").font(.title2.weight(.semibold))
            Form {
                Section("通用") {
                    LabeledContent("运行模式", value: "Demo · 仅本地演示")
                    Picker("演示场景", selection: $model.scenario) { ForEach(DemoScenario.allCases) { Text($0.rawValue).tag($0) } }
                    LabeledContent("自动检查", value: "Demo 关闭；测试时钟手动推进")
                }
                Section("连接") {
                    LabeledContent("OpenAI", value: "在“真实连接”页配置")
                    LabeledContent("X 数据源", value: "已提供实验性免 Token 网页采集")
                }
                Section("通知与隐私") {
                    LabeledContent("系统通知", value: "Demo 禁止 · 仅应用内预览")
                    Text("演示面板保持离线；“真实连接”页由你手动发起请求，结果与演示数据分开保存。")
                        .font(.caption).foregroundStyle(.secondary)
                }
            }.formStyle(.grouped)
            HStack {
